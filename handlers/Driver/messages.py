...
@dp.message_handler(content_types=types.ContentTypes.TEXT, state=OrderMessDriver.wait_choose_command)
async def choose_command_Admin(message: types.Message):
    if message.text not in actions_for_driver:
        await message.reply("Пожалуйста, выберите используя клавиатуру ниже.")
        return

    # "Невыполненные заказы"
    elif message.text == actions_for_driver[0]:
        keyboard = types.ReplyKeyboardMarkup(resize_keyboard=True)
        orders.extend(db.get_orders_for_driver(message.from_user.id))
        for order in orders:
...
async def ready(message: types.Message, state: FSMContext):
    ...
    user_data = await state.get_data()
    db.add_order(message.from_user.id, message.from_user.full_name, user_data['phone_number'], user_data['address'],
                 user_data['price'], user_data['driver'], user_data['driver_id'], datetime.utcnow())
...
@dp.message_handler(Text(equals="Продолжить подтверждение заказа", ignore_case=True),
                    state=OrderMessAdmin.wait_confirm_complite)
