...
from .state import StatesAdmin
from config import dp, db
from datetime import datetime

actions_for_admin = ["Добавить новый заказ"]

@dp.message_handler(Text(equals="Назад в Меню", ignore_case=True), state=OrderMessAdmin.wait_show_command)
...
async def ready(message: types.Message, state: FSMContext):
    ...
    user_data = await state.get_data()
    db.add_order(message.from_user.id, message.from_user.full_name,
                 user_data['phone_number'], user_data['address'], user_data['price'], user_data['driver'], user_data['driver_id'], datetime.utcnow())
